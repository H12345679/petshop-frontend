<template>
  <div class="panel">
    ...
    <div class="address-form card" v-if="showAddressForm">
      ...
      <div class="field">
        <label>所在地区 <span class="req">*</span></label>
        <el-cascader
          v-model="regionCode"
          :options="regionData"
...
/* 级联选择器 - 对齐原有 input 样式 */
.addr-cascader { width: 100%; }
.addr-cascader >>> .el-input__inner {
  padding: 9px 12px; border: 1px solid #d6dbe3; border-radius: 6px; font-size: 13px;
  color: #333; background: #fafbfc; height: auto;
}
.addr-cascader >>> .el-input__inner:focus { border-color: #5b8def; background: #fff; }
</style>
