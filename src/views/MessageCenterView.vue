<template>
  <div class="msg-center-page">
    <AppHeader />

    <div class="container">
      <div class="page-head">
        <h2>消息中心</h2>
        <el-button size="small" @click="markAllRead" :disabled="!hasUnread">全部标为已读</el-button>
      </div>

      <!-- 类型筛选 Tab -->
      <div class="type-tabs">
        <span v-for="t in typeTabs" :key="t.value"
          :class="['tab', { active: activeType === t.value }]"
          @click="activeType = t.value"
        >{{ t.label }}</span>
      </div>

      <!-- 消息列表 -->
      <div v-if="loading" class="loading-wrap">⏳ 加载中…</div>

      <div v-else class="msg-list">
        <div
          v-for="msg in filteredList"
          :key="msg.id"
          class="msg-item"
          :class="{ unread: msg.isRead === 0, read: msg.isRead === 1 }"
          @click="handleRead(msg)"
        >
          <span class="msg-icon">{{ iconForType(msg.type) }}</span>
          <div class="msg-body-right">
            <div class="msg-head">
              <b class="msg-title">{{ msg.title }}</b>
              <span class="msg-type-tag" :class="'type-' + msg.type">{{ typeLabel(msg.type) }}</span>
              <span v-if="msg.isRead === 0" class="unread-dot"></span>
              <span class="msg-time">{{ formatTime(msg.createTime) }}</span>
            </div>
            <div class="msg-content">{{ msg.content }}</div>
            <div v-if="msg.isRead === 1" class="read-tag">已读</div>
          </div>
        </div>

        <div v-if="filteredList.length === 0" class="empty-state">暂无消息</div>
      </div>

      <!-- 分页 -->
      <el-pagination
        v-if="total > size"
        style="margin-top:20px;text-align:center"
        layout="total, prev, pager, next"
        :total="total"
        :page-size="size"
        :current-page.sync="current"
        @current-change="loadMessages"
      />
    </div>

    <AppFooter />
  </div>
</template>

<script>
import { myMessages, readMessage, readAllMessages } from "@/api/modules/message.js";

const TYPE_MAP = { 1: "系统", 2: "订单", 3: "活动", 4: "宠物资讯" };
const TYPE_ICON = { 1: "🔔", 2: "📦", 3: "🎁", 4: "🐾" };

export default {
  name: "MessageCenterView",
  components: { AppHeader: () => import("@/components/AppHeader.vue"), AppFooter: () => import("@/components/AppFooter.vue") },
  data() {
    return {
      list: [],
      loading: true,
      total: 0,
      current: 1,
      size: 15,
      activeType: null,
      typeTabs: [
        { label: "全部", value: null },
        { label: "系统通知", value: 1 },
        { label: "订单消息", value: 2 },
        { label: "活动", value: 3 },
        { label: "宠物资讯", value: 4 },
      ],
    };
  },
  computed: {
    hasUnread() {
      return this.list.some(m => m.isRead === 0);
    },
    filteredList() {
      if (this.activeType == null) return this.list;
      return this.list.filter(m => m.type === this.activeType);
    },
  },
  created() {
    this.loadMessages();
  },
  methods: {
    typeLabel(t) { return TYPE_MAP[t] || "其他"; },
    iconForType(t) { return TYPE_ICON[t] || "🔔"; },

    async loadMessages() {
      this.loading = true;
      try {
        const res = await myMessages({ current: this.current, size: this.size });
        const d = res.data || {};
        this.list = d.records || [];
        this.total = d.total || 0;
      } catch (e) {
        this.$message.error("加载消息失败");
      } finally {
        this.loading = false;
      }
    },

    async handleRead(msg) {
      if (msg.isRead === 1) return;
      try {
        await readMessage(msg.id);
        msg.isRead = 1;
      } catch (e) { /* ignore */ }
    },

    async markAllRead() {
      try {
        await readAllMessages();
        this.list.forEach(m => (m.isRead = 1));
        this.$message.success("已全部标记为已读");
      } catch (e) {
        this.$message.error(e.message || "操作失败");
      }
    },

    formatTime(t) {
      if (!t) return "";
      return t.substring(0, 16).replace("T", " ");
    },
  },
};
</script>

<style scoped>
.msg-center-page { background: #f4f5f7; min-height: 100vh; display: flex; flex-direction: column; }
.container { width: 900px; max-width: 100%; margin: 0 auto; padding: 24px 16px 60px; flex: 1; }
.page-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; }
.page-head h2 { font-size: 20px; font-weight: 700; color: #2c3e50; }
.loading-wrap, .empty-state { text-align: center; padding: 80px 0; color: #999; font-size: 15px; }

/* 类型 Tab */
.type-tabs { display: flex; gap: 4px; background: #fff; border-radius: 10px; padding: 6px; margin-bottom: 16px; }
.type-tabs .tab {
  padding: 6px 16px; border-radius: 8px; font-size: 13px; cursor: pointer;
  color: #666; font-weight: 500; transition: all 0.2s;
}
.type-tabs .tab:hover { background: #f0f2f5; }
.type-tabs .tab.active { background: linear-gradient(135deg, #6b8dd6, #8e37d7); color: #fff; }

/* 消息列表 */
.msg-list { display: flex; flex-direction: column; gap: 10px; }
.msg-item {
  display: flex; gap: 12px; background: #fff; border-radius: 12px;
  padding: 16px 20px; box-shadow: 0 1px 4px rgba(0,0,0,0.04);
  cursor: pointer; transition: all 0.2s;
  border-left: 4px solid transparent;
}
.msg-item:hover { box-shadow: 0 2px 12px rgba(0,0,0,0.08); }
.msg-item.unread { border-left-color: #6b8dd6; background: #f8faff; }
.msg-item.read { opacity: 0.65; }

.msg-icon { font-size: 22px; flex-shrink: 0; margin-top: 2px; }
.msg-body-right { flex: 1; min-width: 0; }
.msg-head { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }
.msg-title { font-size: 14px; color: #333; }
.msg-type-tag {
  font-size: 10px; padding: 2px 8px; border-radius: 100px; font-weight: 600;
  background: #eef4fe; color: #5b8def; flex-shrink: 0;
}
.msg-type-tag.type-2 { background: #fef3e2; color: #e5a452; }
.msg-type-tag.type-3 { background: #fde8e8; color: #e74c3c; }
.msg-type-tag.type-4 { background: #e8f5e9; color: #2e7d32; }

.unread-dot {
  width: 8px; height: 8px; border-radius: 50%; background: #e74c3c; flex-shrink: 0;
}
.msg-time { font-size: 11px; color: #bbb; margin-left: auto; flex-shrink: 0; }
.msg-content { font-size: 13px; color: #666; line-height: 1.5; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.read-tag { font-size: 11px; color: #bbb; margin-top: 4px; }
</style>
